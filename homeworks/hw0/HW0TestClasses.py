...
class ValueIterationTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        self.layout_text = testDict['layout']
        self.grid = layout.Layout([l.strip() for l in self.layout_text.split('\n')])
        self.mdp = valueIterationAgents.PacmanMDP(self.grid)
    ...
    def runAgent(self, moduleDict, numIterations):
        agent = moduleDict['valueIterationAgents'].ValueIterationAgent(self.mdp, discount=self.discount, iterations=numIterations)
        states = self.mdp.getStates()
        actions = list(reduce(lambda a, b: set(a).union(b), [self.mdp.getPossibleActions(state) for state in states]))
        values = {}
        qValues = {}
        policy = {}
        for state in states:
    ...
    def prettyPrint(self, elements, formatString):
        pretty = ''
        states = self.mdp.getStates()
        for ybar in range(self.grid.height):
            y = self.grid.height-1-ybar
            row = []
