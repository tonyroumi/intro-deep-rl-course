...
class PacmanGraphics:
    ...
    def displayValues(self, agent, mdp, currentState = None, message = 'Agent Values'):
        #values = util.Counter()
        values = {}
        policy = {}
        ...
        self.drawValues(mdp, values, policy, currentState, message)

    def drawValues(self, mdp, values, policy, currentState = None, message = 'State Values'):
        valueList = [values[state] for state in values.keys()] + [0.0]
        minValue = min(valueList)
        maxValue = max(valueList)
        for y in range(mdp.layout.width):
            for x in range(mdp.layout.height):
                ...
                screen_y = (y + 1)*self.gridSize
                
                drawSquare(screen_y, screen_x, value, minValue, maxValue, valString, action, False, isExit, isCurrent)
...
def drawSquare(screen_x, screen_y, val, min, max, valStr, action, isObstacle, isTerminal, isCurrent):
    ...
    if not isObstacle:
        text( (screen_x, screen_y), text_color, valStr, "Courier", 12, "normal", "c")
